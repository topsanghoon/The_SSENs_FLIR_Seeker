...
// --- 튜닝 파라미터 (상향/완화) ---
constexpr int COOLDOWN_MS_AFTER_I2C = 12000; // ★ I2C 리셋 후 충분한 쿨다운(리셋 억제)
constexpr int WD_STALL_MIN_MS       = 3000;  // ★ WD 스톨 디바운스 상향 (FFC 오인 방지)
constexpr int FFC_ASSUME_MS         = 1000;  // ★ 연속 discard가 이 시간↑이면 FFC로 간주
constexpr int RESYNC_SOFT_FACTOR    = 3;     // ★ MAX_RESYNC_FAST * 3까지 자연 복구 허용
} // anonymous
...
void IR_CaptureThread::watchdog_run(){
    ...
    // ★ 리셋 남발 억제를 위한 간단한 윈도 카운터
    int resets_last_min = 0;
    auto last_reset_window = std::chrono::steady_clock::now();
    ...
    while (watchdog_running_.load()){
        ...
        // 1분 윈도 갱신
        auto now = std::chrono::steady_clock::now();
        if (std::chrono::duration_cast<std::chrono::seconds>(now - last_reset_window).count() >= 60) {
            resets_last_min = 0;
        ...
        if (cur_fc == last_fc && cur_fc > 0 &&
            ...
            // 리셋 남발 억제: 최근 1분 3회 이상이면 WD 리셋 잠시 억제
            if (resets_last_min >= 3) {
                LOGW(TAG, "Watchdog: resets too frequent in last minute → suppress reset (stalled ~%lldms)",
...
bool IR_CaptureThread::capture_segment(int seg_id){
    ...
    // ★ FFC 추정: 연속 discard 시간 측정
    int discard_run = 0;
    auto discard_run_start = std::chrono::steady_clock::time_point{};
    ...
    while (running_.load() && packets < vospi::PACKETS_PER_SEGMENT) {
        ...
        if (is_discard_packet(packet_buffer_.data())) {
            ...
            // ★ FFC 추정 로직
            if (discard_run++ == 0) {
                discard_run_start = std::chrono::steady_clock::now();
            } else {
                auto dur = std::chrono::duration_cast<std::chrono::milliseconds>(
                    std::chrono::steady_clock::now() - discard_run_start).count();
                if (dur >= FFC_ASSUME_MS) {
                    // FFC 진행중으로 간주: 리셋/재동기 카운터를 리셋하고 자연 복구를 기다린다.
                    resets = 0; expected_line = 0; packets = 0;
                    // usleep 최소화 (가능하면 0) → 스트림 유지
                    continue;
                }
            }

            // 기존 usleep은 최소화(가능하면 제거)
            // ::usleep(vospi::DISCARD_SLEEP_US);

            // 과도 discard가 지속돼도, 먼저 자연 복구를 충분히 시도
            if (++resets >= (vospi::MAX_RESYNC_FAST * RESYNC_SOFT_FACTOR)) {
                // I2C 쿨다운 확인
                bool cooldown_active = false;
                auto now = std::chrono::steady_clock::now();
                if (soft_win_start_.time_since_epoch().count() != 0) {
                ...
                if (!cooldown_active && soft_resets_.fetch_sub(1) > 0) {
                    ...
                    LOGW(TAG, "segment: too many discards/resync → suppress reset (I2C cooldown)");
                }
                resets = 0; expected_line = 0; packets = 0;
            }
            continue;
        ...
        if (line >= vospi::LINES_PER_SEGMENT) {
            // ::usleep(vospi::DISCARD_SLEEP_US); // ★ 최소화
            if (++resets >= (vospi::MAX_RESYNC_FAST * RESYNC_SOFT_FACTOR)) {
                bool cooldown_active = false;
                auto now = std::chrono::steady_clock::now();
                if (soft_win_start_.time_since_epoch().count() != 0) {
                ...
                if (!cooldown_active && soft_resets_.fetch_sub(1) > 0) {
                    ...
                    LOGW(TAG, "segment: too many telemetry skips → suppress reset (I2C cooldown)");
                }
                resets = 0; expected_line = 0; packets = 0;
            }
            continue;
        ...
        if (line < 0 || line != expected_line) {
            ...
            if (++resets >= (vospi::MAX_RESYNC_FAST * RESYNC_SOFT_FACTOR)) {
                ...
                if (!cooldown_active && soft_resets_.fetch_sub(1) > 0) {
                    ...
                    LOGW(TAG, "segment: resync overflow → suppress reset (I2C cooldown)");
                }
                resets = 0;
            }
            continue;
...
bool IR_CaptureThread::read_vospi_packet(uint8_t* pkt){
    ...
    tr.bits_per_word = 8;
    tr.cs_change = 1; // ★ SPI 파라미터 변경은 요청에서 제외 (유지)
